...
CC_SRC_PATH="hyperledger-chaincode/dti"
# import utils
. $CC_SRC_PATH/scripts/envVar.sh
...
packageChaincode() {
  ...
  setGlobals $ORG
  #set -x
  peer lifecycle chaincode package cc.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label $PKG_NAME_${VERSION}
  res=$?
  #set +x
  cat log.txt
...
queryInstalled() {
  ...
  set +x
  cat log.txt
	PACKAGE_ID=$(sed -n "/$PKG_NAME_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log.txt)
...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name $PKG_NAME --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} 
  cat log.txt
  set +x
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name $PKG_NAME --version ${VERSION} --sequence ${VERSION} --output json --init-required 
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name $PKG_NAME $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required 
  res=$?
  set +x
  cat log.txt
...
queryCommitted() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name $PKG_NAME 
    res=$?
    set +x
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n $PKG_NAME $PEER_CONN_PARMS --isInit -c '{"function":"initLedger","Args":[]}' 
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n $PKG_NAME -c '{"Args":["queryAllCars"]}' 
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
