...
function deployCCCV() {
  ...
  scripts/deployCCCV.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function insert() {
  ...
  scripts/insert.sh $1 $2

  if [ $? -ne 0 ]; then
    echo "ERROR !!! Insert action failed"
    exit 1
  fi
...
if [ "$MODE" == "up" ]; then
  ...
  echo
elif [ "$MODE" == "deployCCCV" ]; then
  echo "deploying CV chaincode on channel '${CHANNEL_NAME}'"
  echo
elif [ "$MODE" == "insert" ]; then
  echo "insert CV on channel '${CHANNEL_NAME}'"
  echo
else
  printHelp
  exit 1
